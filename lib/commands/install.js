...
function isHaveBlank(packages){
    for(pack in packages){
        if(packages.hasOwnProperty(pack)){
            var package = packages[pack],
                curr = package.versions[package.current_version];
            if(curr && curr.config.jam && curr.config.jam.blank ){
                return true;
            }
        }
    }
...
exports.installPackages = function (cfg, names, opt, callback) {
    ...
    function (err, packages) {
        ...
        // TODO: write package.json if --save option provided
        var isBlank = isHaveBlank(packages);
        if(isBlank){
            callback();
        }else{
...
exports.installRepo = function (name, range, opt, callback,isBlank) {
    repository.fetch(name, range, opt.repositories,
        function (err, tfile, cdir, v, cfg, from_cache) {
            if (err) {
                return callback(err);
            }
            exports.cpDir(name, v, from_cache, cdir, opt, callback,isBlank);
        }
    );
};
...
exports.cpDir = function (name, v, from_cache, cdir, opt, callback,isBlank) {
    var p = opt.target_dir;
    if(!isBlank){
        p = p + '/' + name;
    }
    function cp() {
